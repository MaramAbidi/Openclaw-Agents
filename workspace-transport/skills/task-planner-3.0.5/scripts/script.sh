#!/usr/bin/env bash
# task-planner v3.0.5 - local task manager and scheduler
...
for t in tasks:
    ...
    due = t.get("due", "") or "-"
    due = due.ljust(10)
    marker = "[done]" if t["status"] == "done" else "[open]"
...
show_help() {
    cat << EOF
task-planner v$VERSION - local task management
