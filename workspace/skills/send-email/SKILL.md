...
2. **Send mail**: Run the script under **workspace** (do not use the path under node_modules):
   ...
   ```
3. **Attachment**: `python C:\Users\hp\.openclaw\workspace\skills\send-email\send_email.py "recipient" "Subject" "Body" "/path/to/file.pdf"`

## Usage examples

```bash
python C:\Users\hp\.openclaw\workspace\skills\send-email\send_email.py 'recipient@example.com' 'Subject' 'Body text'
